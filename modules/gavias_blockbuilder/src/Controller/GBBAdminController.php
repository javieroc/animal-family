...
class GBBAdminController extends ControllerBase {
  ...
  public function gavias_blockbuilder_list(){
    ...
    foreach ($results as $row) {
      ...
      $tmp[] = $row->id;
      $tmp[] = $row->title;
      $tmp[] = t('<a href="@link">Edit</a> | <a href="@link_2">Config</a> |  <a href="@link_3">Delete</a>', array(
  ...
  public function gavias_upload_file(){
    ...
    if(isset($_FILES['upl']) && $_FILES['upl']['error'] == 0){
      ...
      $file_url = str_replace($base_url, '',file_create_url(file_default_scheme(). "://gbb-uploads"). '/' .  $_id .'-'. $_FILES['upl']['name']); 
